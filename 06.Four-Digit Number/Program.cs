using System;
//Write a program that takes as input a four-digit number in format abcd (e.g. 2011) and performs the following:
//Calculates the sum of the digits (in our example 2 + 0 + 1 + 1 = 4).
//Prints on the console the number in reversed order: dcba (in our example 1102).
//Puts the last digit in the first position: dabc (in our example 1201).
//Exchanges the second and the third digits: acbd (in our example 2101).
//The number has always exactly 4 digits and cannot start with 0.
//Examples:
//n	sum of digits	reversed	last digit in front	second and third digits exchanged
//2011	4	1102	1201	2101
//3333	12	3333	3333	3333
//9876	30	6789	6987	9786
class Program
{
    static void Main()
    {
        //write the number.
        Console.Write("Write the number:");
        int number = int.Parse(Console.ReadLine());

        //calculation. deveted by 100 beceouse we are looking for third digit.
        //diveted by % 10 to find is it remainder is 7.
        int a = number / 1000;
        int b = (number / 100)%10;
        int c = (number / 10) % 10;
        int d = number % 10;

        //Calculates the sum of the digits
        int sum = a + b + c + d;
        Console.WriteLine("sum is:{0} ", sum);

        //Prints on the console the number in reversed order
        Console.WriteLine("reversed order is:{0}{1}{2}{3}", d,c,b,a);

        //Prints last digit in front second and third digits exchanged
        Console.WriteLine("reversed order is:{0}{1}{2}{3}", d, a, b, c);
        
    }
}
